...
from task import process_downloadN 
from datetime import datetime

redis_client = redis.Redis(**REDIS_CONFIG)
stream_name = "downloadN_queue"
group_name = "download_group_n"
consumer_name = f"consumer_n_{datetime.now().strftime('%Y%m%d%H%M%S')}"

try:
    redis_client.xgroup_create(stream_name, group_name, id='0-0', mkstream=True)
except redis.exceptions.ResponseError as e:
    if "BUSYGROUP" in str(e):
        print(f"Consumer 그룹 '{group_name}'은 이미 존재합니다.")
        pass
    else:
        raise e
        ...
        if messages:
            for stream, message_list in messages:
                for msg_id, message_data in message_list:
                    ...
                    result = process_downloadN.delay(decoded_message_data) 

                    print(f"[{datetime.now().isoformat()}] - 메시지 {msg_id}를 Celery 작업 {result.id} (process_downloadN)에 할당")
